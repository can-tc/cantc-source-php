...
$q = "select id,patientID,count(*) from updated_winn_rai group by CAN_TC_ID having count(*) > 1";
...
{
	$q = "select id,patientID from updated_winn_rai where patientID = '$ptarr[$i]' order by TxDate ASC";
	$r = mysqli_query($conn,$q) or die("Cannot get patient records -- ".mysqli_error($conn));
	$row = mysqli_fetch_row($r);
	echo "\nPatient is: ".$ptarr[$i]."\n";
	echo "id of the first therapy is: ".$row[0]."\n";
	mysqli_query($conn,
		"update updated_winn_rai set first_thx = 'yes' where patientID = '$ptarr[$i]' and  `id` = '$row[0]'") or die("Cannot update -- ".mysqli_error($conn));
}
